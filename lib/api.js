...
export async function sendChatMessage(message, conversationId = null) {
  ...
  const submittedConvId = task.conversation_id;

  if (!taskId) {
    return {
      conversation_id: submittedConvId,
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    let result;
    try {
      result = await apiFetch(`/api/chat/status/${taskId}`);
    } catch (pollError) {
      // Network blip — retry, don't fail the whole flow
      console.warn(`Poll attempt ${i + 1} failed:`, pollError.message);
      continue;
    }
    ...
    if (result.status === 'completed') {
      if (result.token_usage && typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('token-usage-update', {
          detail: { token_usage: result.token_usage },
        }));
      }
      return {
        conversation_id: result.conversation_id || submittedConvId,
    ...
    if (result.status === 'needs_permission') {
      // Return the permission request to the UI — polling will resume after user chooses
      return {
        conversation_id: result.conversation_id || submittedConvId,
        message: '',
        widgets: result.widgets || [],
        ...
        task_id: taskId,
        pending_doc_id: result.pending_doc_id,
      };
    }
...
export async function sendChatCommand(command, args = '') {
  return apiFetch('/api/chat/command', {
    method: 'POST',
    body: { command, args },
  });
}
...
export async function getTokenUsage() {
  return apiFetch('/api/chat/tokens');
}

export async function sendPermissionChoice(taskId, docId, choice) {
  return apiFetch('/api/chat/permission', {
    method: 'POST',
    body: { task_id: taskId, doc_id: docId, choice },
  });
}

export async function pollTaskStatus(taskId) {
  return apiFetch(`/api/chat/status/${taskId}`);
}
...
export async function addMCPServer(server) {
  return apiFetch('/api/mcp-servers', { method: 'POST', body: server });
}

export async function updateMCPServer(id, updates) {
  return apiFetch(`/api/mcp-servers/${id}`, { method: 'PUT', body: updates });
}
